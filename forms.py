...
class EmailActionBasicForm(forms.Form):
    ...
    canvas_id_column = forms.ChoiceField(
        label='Column to use for target Cavas ID'
    )

    send_to_canvas = forms.BooleanField(
        initial=False,
        required=False,
        label='Send a copy to canvas conversation'
    )
    ...
    def __init__(self, *args, **kargs):
        ...
        self.fields['canvas_id_column'].initial = initial_choice,
        self.fields['canvas_id_column'].choices = \
            [(x, x) for x in self.column_names]
